...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const isAzure = currentRun?.extraction_engine === 'azure_document_intelligence';

  const menuItems = [
    { name: 'Runs', path: '/runs', icon: FileText },
    { name: 'Invoice Debugger', path: `/debugger/${currentRunId || ''}`, icon: Cpu, disabled: !currentRunId },
    { name: 'Candidate Tables', path: `/candidate-tables/${currentRunId || ''}`, icon: Layers, disabled: !currentRunId || isAzure },
    { name: 'OCR Tokens', path: `/ocr-tokens/${currentRunId || ''}`, icon: Activity, disabled: !currentRunId || isAzure },
    { name: 'Selected Table', path: `/selected-table/${currentRunId || ''}`, icon: Grid, disabled: !currentRunId },
    { name: 'Semantic Mapping', path: `/semantic-mapping/${currentRunId || ''}`, icon: MapPin, disabled: !currentRunId || isAzure },
    { name: 'Row Math', path: `/row-math/${currentRunId || ''}`, icon: TrendingUp, disabled: !currentRunId || isAzure },
    { name: 'Quality Gate', path: `/quality-gate/${currentRunId || ''}`, icon: ShieldCheck, disabled: !currentRunId || isAzure },
            ...
            <div className="hidden lg:flex items-center space-x-3 text-xs border-r border-[#30363d] pr-4">
              ...
              <div className="flex items-center space-x-1">
                ...
                <span className="font-mono text-white font-semibold">
                  {currentRun ? (
                    !isAzure && (currentRun.selected_table_available === false || currentRun.status === 'failed' || currentRun.confidence === null || currentRun.confidence === undefined)
                      ? '0.0%'
