...
export const apiClient = {
  ...
  async uploadInvoice(file: File) {
    ...
    const isSafe = isAzure
      ? (conf >= 0.85)
      : (tableUnavailable ? false : (normalizedDetail.quality_gate?.safe_for_erp ?? (normalizedDetail.metadata?.quality_gate?.safe_for_erp ?? false)));
    const qgStatus = isAzure
      ? 'safe'
      : String(normalizedDetail.quality_gate?.status || normalizedDetail.quality_gate?.status_effective || normalizedDetail.metadata?.quality_gate?.status || '').toLowerCase();
    const tokCov = isAzure ? 1.0 : (normalizedDetail.metadata?.token_coverage ?? 0.920);
    const repScore = isAzure ? 1.0 : (normalizedDetail.metadata?.reconstruction_score ?? 0.850);
    ...
    const newRun: RunSummary = {
      ...
      selected_table_id: isAzure ? 'AZURE_TABLE' : (() => {
        if (tableUnavailable) return '—';
        const mainTbl = selectMainTable(normalizedDetail);
        return mainTbl?.table_id || '—';
      })(),
      selected_table_shape: isAzure ? `${normalizedDetail.line_items?.length || 0} Items` : (() => {
      ...
      missing_fields: isAzure ? [] : (normalizedDetail.quality_gate?.missing_fields || (normalizedDetail.metadata?.quality_gate?.missing_fields || [])),
      row_math_status: isAzure ? 'pass' : (normalizedDetail.quality_gate?.row_math_status || (normalizedDetail.metadata?.quality_gate?.row_math_status || 'pass')),
      is_demo: false,
