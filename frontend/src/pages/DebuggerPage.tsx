  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8.5rem)] gap-4 select-none">
      ...
      <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-3 flex flex-wrap items-center justify-between gap-3 shrink-0">
        ...
        {/* Visual Layer Toggles / Azure Info Banner */}
        {isAzure ? (
          <div className="flex items-center space-x-2 text-xs font-mono text-gray-400">
            <span className="px-2.5 py-1 rounded bg-[#1f242c] border border-[#30363d] text-[#00f0ff] font-semibold">
              Azure Document Intelligence Mode
            </span>
            <span>OCR geometry and layout overlays are disabled for this run.</span>
          </div>
        ) : (
      ...
      <div className="flex-1 flex gap-4 min-h-0 overflow-hidden">
        ...
        <div className="flex-1 bg-[#161b22] border border-[#30363d] rounded-lg flex flex-col overflow-hidden relative">
          ...
          <div
            ref={containerRef}
            onMouseDown={isAzure ? undefined : handleMouseDown}
            onMouseMove={isAzure ? undefined : handleMouseMove}
            onMouseUp={isAzure ? undefined : handleMouseUp}
            onMouseLeave={isAzure ? undefined : handleMouseUp}
            className={`flex-1 overflow-hidden relative flex items-center justify-center bg-[#0d1117] ${isAzure ? '' : (isPanning ? 'cursor-grabbing' : 'cursor-grab')}`}
          >
            {activeRun ? (
              <>
                ...
                <div
                  style={{
                    transform: isAzure ? 'none' : `scale(${zoom}) translate(${pan.x}px, ${pan.y}px)`,
                    transformOrigin: 'center center',
                    transition: isPanning ? 'none' : 'transform 0.1s ease-out',
                    width: isAzure ? '100%' : `${baseWidth}px`,
                    height: isAzure ? '100%' : `${baseHeight}px`
                  }}
                  className={`relative select-none ${isAzure ? 'flex items-center justify-center p-4 h-full w-full' : ''}`}
                >
                  ...
                  <img
                    ...
                    className={`pointer-events-none select-none ${isAzure ? 'max-w-full max-h-full object-contain shadow-2xl rounded-lg border border-[#30363d]' : 'w-full h-full'}`}
                    draggable={false}
                    onLoad={handleImageLoad}
                  />
                </div>

                {/* 2. SVG OVERLAY RECTANGLES LAYER & HUDS (Disabled for Azure Document Intelligence) */}
                {!isAzure && (
                  <>
                    <svg
                  ...
                  {isAzure ? (
                    <div className="space-y-4 font-mono text-xs">
                      <div className="border-b border-[#30363d] pb-2">
                        <span className="text-[10px] text-gray-500 uppercase font-bold block">Azure Extraction</span>
                        <h4 className="text-[#00f0ff] text-sm font-bold">Document Metadata</h4>
                      </div>

                      <div className="space-y-3 font-mono">
                        <div>
                          <span className="text-[10px] text-gray-500 block uppercase font-mono">Seller</span>
                          <span className="text-white font-sans font-semibold text-xs block truncate" title={runDetail?.seller_name || '—'}>
                            {runDetail?.seller_name || '—'}
                          </span>
                        </div>

                        <div>
                          <span className="text-[10px] text-gray-500 block uppercase font-mono">Buyer</span>
                          <span className="text-white font-sans font-semibold text-xs block truncate" title={runDetail?.buyer_name || '—'}>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <span className="text-[10px] text-gray-500 block uppercase font-mono">Invoice No</span>
                            <span className="text-white font-semibold">{runDetail?.invoice_number || '—'}</span>
                          </div>
                          <div>
                            <span className="text-[10px] text-gray-500 block uppercase font-mono">Date</span>
                            <span className="text-white font-semibold">{runDetail?.invoice_date || runDetail?.date || '—'}</span>
                          </div>
                        </div>

                        <div className="grid grid-cols-2 gap-2 border-t border-[#30363d] pt-2.5">
                          <div>
                            <span className="text-[10px] text-gray-500 block uppercase font-mono">Subtotal</span>
                            <span className="text-white font-semibold">₹{runDetail?.subtotal?.toFixed(2) || '0.00'}</span>
                          </div>
                          <div>
                            <span className="text-[10px] text-gray-500 block uppercase font-mono">Discount</span>
                            <span className="text-white font-semibold">₹{runDetail?.discount?.toFixed(2) || '0.00'}</span>
                          </div>
                        </div>

                        <div className="grid grid-cols-3 gap-1.5">
                          <div>
                            <span className="text-[10px] text-gray-500 block uppercase font-mono">SGST</span>
                            <span className="text-white font-semibold">₹{runDetail?.sgst?.toFixed(2) || '0.00'}</span>
                          </div>
                          <div>
                            <span className="text-[10px] text-gray-500 block uppercase font-mono">CGST</span>
                            <span className="text-white font-semibold">₹{runDetail?.cgst?.toFixed(2) || '0.00'}</span>
                          </div>
                          <div>
                            <span className="text-[10px] text-gray-500 block uppercase font-mono">IGST</span>
                            <span className="text-white font-semibold">₹{runDetail?.igst?.toFixed(2) || '0.00'}</span>
                          </div>
                        </div>

                        <div className="border-t border-[#30363d] pt-2.5">
                          <span className="text-[10px] text-gray-500 block uppercase font-mono">Grand Total</span>
                          <span className="text-[#00f0ff] font-bold text-sm">₹{runDetail?.grand_total?.toFixed(2) || '0.00'}</span>
                        </div>

                        <div className="border-t border-[#30363d] pt-2.5">
                          <span className="text-[10px] text-gray-500 block uppercase font-mono">Confidence</span>
                          <span className={`font-bold text-xs ${
                            (runDetail?.confidence ?? 0) >= 0.85 ? 'text-emerald-400' : 'text-amber-400'
                          }`}>
                            {typeof runDetail?.confidence === 'number'
                              ? `${(runDetail.confidence * 100).toFixed(1)}%`
                              : '—'}
                          </span>
                        </div>
                      </div>
      ...
      {isAzure ? (
        runDetail?.line_items && runDetail.line_items.length > 0 ? (
          <div className="bg-[#161b22] border border-[#30363d] rounded-lg overflow-hidden shrink-0">
            <div className="bg-[#0d1117] px-4 py-2 border-b border-[#30363d] flex items-center justify-between text-xs font-mono">
              <div className="flex items-center space-x-2">
                <span className="text-gray-500 uppercase font-mono">Azure Extracted Items:</span>
                <strong className="text-white font-mono">Line Items Table</strong>
                <span className="text-gray-600 font-mono">({runDetail.line_items.length} Items)</span>
              </div>
            </div>
            ...
            <div className="overflow-x-auto max-h-48 custom-scrollbar">
              <table className="w-full text-left text-xs font-mono">
                <thead>
                  <tr className="bg-[#0d1117] border-b border-[#30363d] text-gray-400 font-bold uppercase text-[10px]">
                    <th className="p-2 px-3 border-r border-[#30363d] text-center w-8">#</th>
                    <th className="p-2 px-3 border-r border-[#30363d]">Name</th>
                    <th className="p-2 px-3 border-r border-[#30363d]">Pack</th>
                    <th className="p-2 px-3 border-r border-[#30363d]">Batch</th>
                    <th className="p-2 px-3 border-r border-[#30363d]">Expiry</th>
                    <th className="p-2 px-3 border-r border-[#30363d]">HSN</th>
                    <th className="p-2 px-3 border-r border-[#30363d] text-right">Qty</th>
                    <th className="p-2 px-3 border-r border-[#30363d] text-right">Free</th>
                    <th className="p-2 px-3 border-r border-[#30363d] text-right">MRP</th>
                    <th className="p-2 px-3 border-r border-[#30363d] text-right">Rate</th>
                    <th className="p-2 px-3 border-r border-[#30363d] text-right">Disc %</th>
                    <th className="p-2 px-3 border-r border-[#30363d] text-right">GST %</th>
                    <th className="p-2 px-3 text-right">Amount</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#30363d]">
                  {runDetail.line_items.map((item: any, idx: number) => (
                    <tr key={idx} className="hover:bg-[#1f242c] text-gray-200">
                      <td className="p-2 px-3 border-r border-[#30363d] text-gray-500 text-[10px] text-center">
                        {(idx + 1).toString().padStart(2, '0')}
                      </td>
                      <td className="p-2 px-3 border-r border-[#30363d] text-white font-sans font-semibold max-w-[180px] truncate">
                        {item.name || '—'}
                      </td>
                      <td className="p-2 px-3 border-r border-[#30363d]">{item.pack || '—'}</td>
                      <td className="p-2 px-3 border-r border-[#30363d]">{item.batch || '—'}</td>
                      <td className="p-2 px-3 border-r border-[#30363d]">{item.expiry || '—'}</td>
                      <td className="p-2 px-3 border-r border-[#30363d]">{item.hsn || '—'}</td>
                      <td className="p-2 px-3 border-r border-[#30363d] text-right">{item.quantity ?? '—'}</td>
                      <td className="p-2 px-3 border-r border-[#30363d] text-right">{item.free_quantity ?? '—'}</td>
                      <td className="p-2 px-3 border-r border-[#30363d] text-right">
                        {typeof item.mrp === 'number' ? `₹${item.mrp.toFixed(2)}` : '—'}
                      </td>
                      <td className="p-2 px-3 border-r border-[#30363d] text-right">
                        {typeof item.rate === 'number' ? `₹${item.rate.toFixed(2)}` : '—'}
                      </td>
                      <td className="p-2 px-3 border-r border-[#30363d] text-right">
                        {typeof item.discount === 'number' ? `${item.discount.toFixed(1)}%` : '—'}
                      </td>
                      <td className="p-2 px-3 border-r border-[#30363d] text-right">
                        {typeof item.gst_percent === 'number' ? `${item.gst_percent.toFixed(1)}%` : '—'}
                      </td>
                      <td className="p-2 px-3 text-right text-emerald-400 font-semibold font-mono">
          ...
          <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-6 shrink-0 flex flex-col items-center justify-center text-center space-y-2">
            <span className="text-gray-500 text-xs font-mono">No line items extracted from Azure.</span>
          </div>
        )
      ) : (
        activeRun?.selected_table_available === false || selectedTableUnavailable ? (
            ...
            <div className="bg-[#161b22] border border-[#30363d] rounded-lg overflow-hidden shrink-0">
              <div className="bg-[#0d1117] px-4 py-2 border-b border-[#30363d] flex items-center justify-between text-xs font-mono">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-gray-600 font-mono">({selectedTable.rows} Rows x {selectedTable.cols} Columns)</span>
                </div>
